...
from .serializers import AssessmentSerializer, CourseSerializer, SubmitResultSerializer, \
...
class SubmittedResultViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if dro_role:
            return SubmittedResult.objects.filter(lecturer__profiles__department=user.profiles.department, result_status='P_D')
        if fro_role:
            return SubmittedResult.objects.filter(lecturer__profiles__department__faculty=user.profiles.department.faculty, result_status='P_F')
        if co_role:
